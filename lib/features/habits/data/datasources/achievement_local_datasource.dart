...
class AchievementLocalDataSourceImpl implements AchievementLocalDataSource {
  ...
  Future<List<Achievement>> getCachedAchievements() async {
    try {
      ...
      // If there's an error reading cached data, return empty list
      return [];
    }
  }

  @override
  Future<void> cacheUserAchievements(String userId, List<Achievement> achievements) async {
    try {
  ...
  Future<UserStats?> getCachedUserStats(String userId) async {
    try {
      ...
      // If there's an error reading cached data, return null
      return null;
    }
  }
  ...
  Future<void> clearAllCachedData() async {
    try {
      ...
      // Filter keys related to achievements and stats
  ...
  Future<void> clearUserData(String userId) async {
    try {
      final userAchievementsKey = '$_userAchievementsPrefix$userId';
      final userStatsKey = '$_userStatsPrefix$userId';
      
      await sharedPreferences.remove(userAchievementsKey);
      await sharedPreferences.remove(userStatsKey);
    } catch (e) {
      throw Exception('Failed to clear user data: ${e.toString()}');
    }
  }

  // Helper method to get cache size info
  ...
  // Helper method to check if cache is valid (not too old)
  bool _isCacheValid(String key, {Duration maxAge = const Duration(hours: 24)}) {
    try {
  ...
  // Helper method to update cache timestamp
  ...
  // Method to cache achievements with timestamp
  Future<void> cacheAchievementsWithTimestamp(List<Achievement> achievements) async {
    await cacheAchievements(achievements);
    await _updateCacheTimestamp(_achievementsKey);
  }

  // Method to cache user achievements with timestamp
  Future<void> cacheUserAchievementsWithTimestamp(String userId, List<Achievement> achievements) async {
    await cacheUserAchievements(userId, achievements);
    final key = '$_userAchievementsPrefix$userId';
    await _updateCacheTimestamp(key);
  }

  // Method to cache user stats with timestamp
  Future<void> cacheUserStatsWithTimestamp(String userId, UserStats stats) async {
    await cacheUserStats(userId, stats);
    final key = '$_userStatsPrefix$userId';
    await _updateCacheTimestamp(key);
  }

  // Method to get valid cached achievements
  Future<List<Achievement>> getValidCachedAchievements() async {
    if (!_isCacheValid(_achievementsKey)) {
      return [];
    }
    return getCachedAchievements();
  }

  // Method to get valid cached user achievements
  Future<List<Achievement>> getValidCachedUserAchievements(String userId) async {
    final key = '$_userAchievementsPrefix$userId';
  ...
  // Method to get valid cached user stats
